...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Creating 4000 Reviews"))
        reviews = ReviewFactory.create_batch(4000)

        self.stdout.write(
            self.style.SUCCESS("Creating 10 Answers for each review (takes a while)")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"DB Records: {Review.objects.count()} Reviews, "
